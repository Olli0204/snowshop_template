{block name='layout-header-nav-icons'}
    {nav id="shop-nav" right=true class="nav-right order-lg-last nav-icons" role="navigation" aria-label="{lang key='shopNavigation'}"}
        ...
        {block name='layout-header-nav-icons-include-currency-dropdown'}
            ...
            {/if}
        {/block}
        {* Einfügen des Languagedropdowns in die primäre Headerleiste *}
        {block name='layout-header-nav-icons-include-language-globe-dropdown'}
            {include file='layout/header_nav_language_top_bar.tpl'}
        {/block}
        {* Eigentliches Sprachen Dropdown fürs Handy entfernt *}
